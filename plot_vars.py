...
for year in range(2001, 2011, 1):
    ...
    f2 = nc.Dataset('%s%sOutputs/gw_on/cable_out_%s.nc' %(path, case2, year), 'r')
    ...
    if var_name == 'SoilMoist':
        if year == 2001:
            ...
            var2_aclm = np.average(((f2.variables['SoilMoist'][:,0,:,:]*0.022+f2.variables['SoilMoist'][:,1,:,:]*0.058 \
        	      + f2.variables['SoilMoist'][:,2,:,:]*0.154+f2.variables['SoilMoist'][:,3,:,:]*0.409 \
        	      + f2.variables['SoilMoist'][:,4,:,:]*1.085+f2.variables['SoilMoist'][:,5,:,:]* 2.872)/4.6),axis = 0)
        else:
        	var1_aclm = var1_aclm + np.average(((f1.variables['SoilMoist'][:,0,:,:]*0.022+f1.variables['SoilMoist'][:,1,:,:]*0.058 \
        	      + f1.variables['SoilMoist'][:,2,:,:]*0.154+f1.variables['SoilMoist'][:,3,:,:]*0.409 \
        	      + f1.variables['SoilMoist'][:,4,:,:]*1.085+f1.variables['SoilMoist'][:,5,:,:]* 2.872)/4.6),axis = 0)
        	var2_aclm = var2_aclm + np.average(((f2.variables['SoilMoist'][:,0,:,:]*0.022+f2.variables['SoilMoist'][:,1,:,:]*0.058 \
        	      + f2.variables['SoilMoist'][:,2,:,:]*0.154+f2.variables['SoilMoist'][:,3,:,:]*0.409 \
        	      + f2.variables['SoilMoist'][:,4,:,:]*1.085+f2.variables['SoilMoist'][:,5,:,:]* 2.872)/4.6),axis = 0)
    else:
        if year == 2001:
            var1_aclm = np.average(f1.variables[var_name][:,:,:],axis = 0)*3600*24
            var2_aclm = np.average(f2.variables[var_name][:,:,:],axis = 0)*3600*24
        else:
            var1_aclm = var1_aclm + np.average(f1.variables[var_name][:,:,:],axis = 0)*3600*24
            var2_aclm = var1_aclm + np.average(f2.variables[var_name][:,:,:],axis = 0)*3600*24

var1_avg = var1_aclm/10.
var2_avg = var2_aclm/10.
...
for year in range(2001, 2011, 1):
    ...
    if var_name == 'SoilMoist':
        ...
        var2 = (f2.variables['SoilMoist'][:,0,:,:]*0.022+f2.variables['SoilMoist'][:,1,:,:]*0.058 \
        	     + f2.variables['SoilMoist'][:,2,:,:]*0.154+f2.variables['SoilMoist'][:,3,:,:]*0.409 \
        	     + f2.variables['SoilMoist'][:,4,:,:]*1.085+f2.variables['SoilMoist'][:,5,:,:]* 2.872)/4.6
    else:
        var1 = f1.variables[var_name][:,:,:]
        var2 = f2.variables[var_name][:,:,:]
